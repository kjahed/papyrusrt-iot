...
import org.eclipse.papyrusrt.codegen.cpp.rts.UMLRTRuntime.UMLRTSignalRegistry;
...
public class ProtocolGenerator extends AbstractElementGenerator {
	 ...
	 * @return The C++ class.
	 */
	protected CppClass getRole(Output kind) {
		...
		Function getRegistryFunc = new Function(new HeaderFile("umlrtsignalregistry"),
				LinkageSpec.STATIC, UMLRTSignalRegistry.getType(), "UMLRTSignalRegistry::getRegistry");


		Variable registry = new Variable(UMLRTSignalRegistry.getType().ref(),
				"signalRegistry", new FunctionCall(getRegistryFunc));

		ElementAccess registryAccess = new ElementAccess(registry);

		MemberFunction registerInSignalFunc = new MemberFunction(PrimitiveType.VOID, "registerInSignal");
		MemberFunction registerOutSignalFunc = new MemberFunction(PrimitiveType.VOID, "registerOutSignal");
		...
		for (Signal signal : RTSModelLibraryUtils.getAllUserSignals(protocol)) {
			...
			MemberFunctionCall registerCall;
			if (signal.getKind() == ProtocolBehaviourFeatureKind.IN) {
				if (kind == CppCodePattern.Output.ProtocolBaseRole)
					registerCall = new MemberFunctionCall(registryAccess, registerInSignalFunc);
				else
					registerCall = new MemberFunctionCall(registryAccess, registerOutSignalFunc);
			} else if (signal.getKind() == ProtocolBehaviourFeatureKind.OUT) {
				if (kind == CppCodePattern.Output.ProtocolBaseRole)
					registerCall = new MemberFunctionCall(registryAccess, registerOutSignalFunc);
				else
					registerCall = new MemberFunctionCall(registryAccess, registerInSignalFunc);
			} else {
				continue; // FIXME: support InOut
			}

			registerCall.addArgument(new StringLiteral(protocol.getName()));
			registerCall.addArgument(new StringLiteral(signal.getName()));
			registerCall.addArgument(new ElementAccess(sigEnumerator));
			registerCall.addArgument(new AddressOfExpr(new ElementAccess(payload)));

			ctor.add(registerCall);
		}
