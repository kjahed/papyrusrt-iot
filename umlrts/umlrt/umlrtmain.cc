...
const char * UMLRTMain::address = NULL;
...
static UMLRTMain::t_usage_option_help optionhelp[] = {
        ...
        { "address",        'i', "<address>",   "Specify the local address" },
...
/*static*/ void UMLRTMain::setArgs( int argc_, char * const * argv_ )
{
   static struct option options[] = {
    ...
    while (optionsvalid && ((optchar = getopt_long( argc_, argv_, ":h:i:c:D:C:T:t:N:S:n:F:L:f:m:Msul", options, &longindex)) != -1) && !userargsfound)
    {
    ...
    if (deploymentfile)
    {
        ...
        if (!UMLRTDeploymentMap::fromFile(deploymentfile))
        {
            printf("ERROR: failed to create deployment map. Exiting.\n");
            usage(argv_[0]);
        }
        UMLRTDeploymentMap::debugOutputCaspuleToControllerMap();
        UMLRTDeploymentMap::debugOutputControllerToHostMap();
    }
    ...
    if (displaymodel)
    {
        ...
        {
            UMLRTController * controller = (UMLRTController *)UMLRTDeploymentMap::getFirstController();
            if (controller == NULL)
            {
